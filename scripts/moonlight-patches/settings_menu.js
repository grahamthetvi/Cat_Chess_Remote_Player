...
const TUNNEL_MAX_FPS = 30;
function isOversizedForTunnel(settings) {
    if (!settings) {
        return false;
    }
    if (settings.videoSize === "720p" || settings.videoSize === "1080p" || settings.videoSize === "1440p" || settings.videoSize === "4k" || settings.videoSize === "native") {
        return true;
    }
    ...
    if (Number(settings.bitrate) > TUNNEL_MAX_BITRATE) {
        return true;
    }
    if (Number(settings.fps) > TUNNEL_MAX_FPS) {
        return true;
    }
    return false;
}
function applyTunnelSafeClamp(settings) {
    // Cloudflare Tunnel WebSocket cannot sustain 720p+ well: host logs show
    // "Video decode unit queue overflow" / purple blank video while audio/input still work.
    settings.dataTransport = "websocket";
    settings.videoCodec = "h264";
    settings.hdr = false;
    settings.canvasRenderer = true;
    if (isOversizedForTunnel(settings) || settings.videoSize !== "custom") {
        settings.videoSize = "custom";
        settings.videoSizeCustom = { width: 960, height: 540 };
    }
    if (!Number.isFinite(settings.bitrate) || settings.bitrate > TUNNEL_MAX_BITRATE || settings.bitrate < 500) {
        settings.bitrate = 1500;
    }
    if (!Number.isFinite(settings.fps) || settings.fps > TUNNEL_MAX_FPS || settings.fps < 15) {
        settings.fps = 30;
    }
    if (!Number.isFinite(settings.videoFrameQueueSize) || settings.videoFrameQueueSize < 3) {
        settings.videoFrameQueueSize = 5;
    }
    settings._tunnelSchema = TUNNEL_SCHEMA;
    return settings;
}
export function getLocalStreamSettings(defaultSettings) {
    ...
    try {
        ...
        if (json) {
            ...
            // Stale browser settings often keep 1080p/10Mbps from first install and
            // override the Admin role's tunnel-safe 720p defaults.
            if (loaded._tunnelSchema !== TUNNEL_SCHEMA || isOversizedForTunnel(loaded)) {
                const preserved = {};
                for (const key of ["language", "sidebarEdge", "mouseMode", "mouseScrollMode", "touchMode", "localCursorSensitivity", "controllerConfig", "pageStyle"]) {
                    if (loaded[key] !== undefined) {
                        preserved[key] = loaded[key];
                    }
                }
                settings = deepMerge(settings, preserved);
                settings = applyTunnelSafeClamp(settings);
...
export function setLocalStreamSettings(settings) {
    const next = applyTunnelSafeClamp(deepClone(settings));
