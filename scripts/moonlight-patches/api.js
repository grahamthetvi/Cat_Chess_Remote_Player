...
function isHostLocalMoonlight() {
    const host = String(window.location.hostname || "");
    return host === "127.0.0.1" || host === "localhost";
}
function fetchArcadeMoonlightCredentials() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Host tab talks to the portal on loopback; remote clients use same-origin.
            const url = isHostLocalMoonlight()
                ? "http://127.0.0.1:3000/api/moonlight-host-nudge"
                : "/api/moonlight-credentials";
...
function startHostReloginWatcher() {
    if (!isHostLocalMoonlight()) {
        return;
    }
    let lastNudge = Number(sessionStorage.getItem("mlHostNudgeAt") || "0");
    window.setInterval(() => __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch("http://127.0.0.1:3000/api/moonlight-host-nudge", {
                cache: "no-store"
            });
            if (!response.ok) {
                return;
            ...
            const data = yield response.json();
            const nudgeAt = Number(data && data.nudgeAt) || 0;
            if (nudgeAt <= lastNudge) {
                return;
            }
            lastNudge = nudgeAt;
            sessionStorage.setItem("mlHostNudgeAt", String(nudgeAt));
            // Client typed the wrong Moonlight password → refresh host UI and sign in again.
            const host_url = buildUrl("/api");
            const api = { host_url, bearer: null, user: null, role: null };
            try {
                yield apiLogout(api);
            }
            catch (_b) {
                // Ignore logout failures (already logged out).
            }
            if (data.name && data.password) {
                if (yield apiLogin(api, { name: data.name, password: data.password })) {
                    yield finishLogin(api);
                }
            }
            window.location.reload();
        }
        catch (_a) {
            // Portal may be restarting; ignore.
        }
    }), 2000);
}
startHostReloginWatcher();
function finishLogin(api) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(yield apiAuthenticate(api))) {
...
export function tryLogin() {
    return __awaiter(this, void 0, void 0, function* () {
        const host_url = buildUrl("/api");
        let api = { host_url, bearer: null, user: null, role: null };
        const stored = yield fetchArcadeMoonlightCredentials();
        // Silent auto-login when credentials are available (portal secrets / host nudge).
        if (stored && stored.name && stored.password) {
            if (yield apiLogin(api, { name: stored.name, password: stored.password })) {
                return yield finishLogin(api);
            }
        }
        const prompt = new ApiUserPasswordPrompt(stored);
        ...
        if (yield apiLogin(api, userAuth)) {
            ...
            yield showMessage("Credentials are not Valid", {
                okLabel: "You probably have the right info, try again."
            });
            // Remote clients: reload so stored credentials can auto-sign-in.
            // Host tab is also nudged via /api/moonlight-host-nudge (proxy detects this 401).
            window.location.reload();
            return null;
        }
    });
